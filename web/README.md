...
An optional full-canvas Designer view provides local WYSIWYG editing without replacing the source editor. Existing source-linked fields can be selected, dragged, nudged, duplicated, deleted, and edited through a property inspector. Text, Code 128, QR code, box, and line elements can be dragged from the toolbox onto the label. Designer changes are emitted as single-range Monaco edits, so ZPL remains authoritative and normal undo/redo continues to work. Grid snapping, zoom-to-fit, multi-label switching, transform-aware coordinates, and a compact mobile layout are included.
